package com.app.purecookbook.purecookbook.db;
...
public class CooksDBManager {
    ...
    private CookbookInfo.Result.Data  data;
     ...
     * 增添数据
     */
    public void insertData(CookbookInfo.Result.Data data) {
     ...
     * @param data
     */
    public void delData(CookbookInfo.Result.Data data) {
        if (data == null) {
     ...
     * @param isLike 是否收藏
     */
    public void updateData(CookbookInfo.Result.Data data, boolean isLike) {
        String textSql = "update " + CooksDBHelper.TABLE_TEXT + " set "
                + CooksDBHelper.MY_LIKE + "='" + (isLike ? 1 : 0) + "' where " + CooksDBHelper.ID + "=" + data.getId();
        db.execSQL(textSql);
    }

    public CookbookInfo getData(boolean isHistory, boolean isLike) {
        ...
        CookbookInfo.Result result = new CookbookInfo.Result();
        info.setResult(result);
        List<CookbookInfo.Result.Data> datas = new ArrayList<>();
        result.setData(datas);
        CookbookInfo.Result.Data data;
        while (textCursor.moveToNext()) {
            data = new CookbookInfo.Result.Data();
            ...
            List<CookbookInfo.Result.Data.Steps> stepses = new ArrayList<>();
            data.setSteps(stepses);
            CookbookInfo.Result.Data.Steps steps;
            ...
            while (imgsCursor.moveToNext()) {
                steps = new CookbookInfo.Result.Data.Steps();
    ...
    public CookbookInfo.Result.Data getData() {
        return data;
    }

    public void setData(CookbookInfo.Result.Data data) {
        this.data = data;
    }
